...
def embed_text(text):
    ...
    if len(text) > 100000:
        print(f"  ⚠ Text is very long ({len(text)} characters), truncating to 100,000 characters")
        text = text[:100000]
...
for filename in os.listdir(UNCLEANED_DIR):
    if filename.endswith((".eml", ".txt", ".md")):
        ...
        try:
            ...
            try:
                print(f"  Generating embedding and uploading to Pinecone")
                ...
                print(f"  Metadata prepared with {len(metadata)} fields")
                print(f"  Uploading to Pinecone index: {INDEX_NAME}")
                
                upload_start = time.time()
                index.upsert([(filename, embedding, metadata)])
                print(f"  ✓ Successfully uploaded to Pinecone in {time.time() - upload_start:.2f} seconds")
